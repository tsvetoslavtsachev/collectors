...
Сетивата на обсерваторията. Всеки колектор дърпа данни от свободен първоизточник и ги
...
**Лицето на oil (КОКПИТ Вълна 1, О4/B2):** рендерът носи ДЕСКРИПТИВЕН режимен речник —
`РЕЖИМ: ИЗЧАКВАНЕ / НЕРАВНОВЕСИЕ / РАЗТОВАРВАНЕ`, не predictive „ТЕЗАТА ДОКАЗАНА/ОПРОВЕРГАНА"
(уредът е дескриптивен) + О4 бадж `НАБЛЮДЕНИЕ · дескриптивен режим`. Вътрешните
verdict-ключове (PROVEN/WAIT/DEAD) остават за цвят/логика; сменя се само лицето. Пълният
редизайн (режимен изход) = oil-v2, отделна писта.
...
- `vrm.yml` — VRM серии + brain M-модулите (съб)
  - след мозъка пише и `signals/current.json` в data-core за signals-registry (vrm-core, vrm-mid, kill-switch и dollar-correlation от дневните DXY/SPY в price-archive; `collectors/vrm/export_signals.py`, само чете state и цени)
